...
if ($device == "mobile") {
	$size = 'mic';
	$gallery_sizes = ["mic","min","med"];
} elseif ($device == "desktop") {
	$size = 'min';
	$gallery_sizes = ["min","med","big"];
}


$images = ImageList::from_folder("gallery/");
...
foreach($images as $key=>$image) {
	...
	echo "<div style='border:solid 2px #aeaeae;cursor:pointer;margin:0.2em;float:left;' onclick='$(\".tiles\").hide();$(\".gallery\").show();$(\".photor".$key."\").show();$(\".photor".$key."\").photor();'><img src='generator.php?name=".$image."&size=".$size."'></div>";
...
foreach($images as $key=>$image) {
    ...
    echo '
    <div class="photor'.$key.'" style="display:none;">
        <div class="photor__viewport">
            <div class="photor__viewportLayer">';

            foreach ($gallery_sizes as $gallery_size) {

                echo "<img src='generator.php?name=".$image."&size=".$gallery_size."' data-thumb='generator.php?name=".$image."&size=mic'>";
    ...
    echo '  </div>
        ...
        <div class="photor__thumbs"><div class="photor__thumbsWrap"></div></div>
    </div>';
...
echo "<script>
        $(document).keyup(function(e) {
          if (e.key === 'Escape') {
            for (let i=0;i<10;i++) $(\".photor\"+i).hide();
            $(\".gallery\").hide();
            $(\".tiles\").show();
          }
        });
